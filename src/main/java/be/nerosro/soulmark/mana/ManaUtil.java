...
public final class ManaUtil {
     ...
     * If the player doesn't have enough, fires a {@link ManaOverspendEvent}.
     * If a job mod allows the overspend, mana goes negative and the method returns true.
    ...
    public static boolean trySpend(Player player, float cost) {
        ...
        // Not enough mana — ask if any mod allows overspending
        ManaOverspendEvent event = new ManaOverspendEvent(player, cost, mana.getCurrentMana());
        NeoForge.EVENT_BUS.post(event);

        if (event.isOverspendAllowed()) {
            mana.forceSpend(cost);
            return true;
        }
    ...
    /**
     * Returns the player's current mana as a fraction of their max pool.
     * Typically 0.0 to 1.0, but returns negative values if the player is in depth (through Elemancy).
     */
    ...
    /**
     * Returns true if the player's mana is negative (in depth).
     */
    public static boolean isInDepth(Player player) {
        return getMana(player).getCurrentMana() < 0;
    }

    /**
     * Returns how far below zero the player's mana is (always >= 0).
     * Returns 0 if mana is non-negative.
     */
    public static float getDepth(Player player) {
        return Math.max(0, -getMana(player).getCurrentMana());
    }
}
